...
{
public:
    ARR2D() : w(0), h(0), f_init(0), pItem(0) { }
    ~ARR2D()
    {
        destroy();
    }

    ARR2D(ARR2D<T> &src): w(0),h(0),f_init(0),pItem(0)
    {
        if (src.w <= 0 || src.h <= 0)
        {
            throw 1;
    ...
    int inline width() { return w; }

    int inline height() { return h; }
    ...
    virtual int saveToFile(char *fn)
    {
        if (!f_init)
            return 1;
...
private:
    ...
    static inline T getArrValue(T* arr, int x,int y,int w)
    {
        return arr[y*w+x];
    }

    static inline void setArrValue(T* arr, int x,int y,int w,T value)
    {
        arr[y*w+x] = value;
    }
};
